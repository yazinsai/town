...
export default function QuickResponse({ agent, onClose }: QuickResponseProps) {
  ...
  async function handleAnswer() {
    ...
    try {
      ...
      // If there's freeform text and a question without a selected answer, use freeform
  ...
  async function handleMessage() {
    ...
    setSubmitting(true);
    try {
      await respondToAgent(agent.id, { type: "message", message: freeformText });
      onClose();
    } catch (err) {
      console.error("Failed to respond:", err);
    } finally {
      ...
      {isPermission && agent.pendingPermission && (
        <div>
          ...
          <div style={{ display: "flex", gap: "8px" }}>
            <PixelButton onClick={() => handlePermission(true)} disabled={submitting}>
              APPROVE
            </PixelButton>
            <PixelButton
              variant="danger"
      ...
      {!isQuestion && !isPermission && (
        <div>
          ...
          <PixelInput
            ...
            style={{ marginBottom: "8px" }}
          />
          <PixelButton onClick={handleMessage} disabled={submitting}>
            SEND
          </PixelButton>
        </div>
      )}
